...
test.describe('Agendamentos - Finalizar agendamento Pendente no mês', () => {
  ...
  test('Deve percorrer o mês até encontrar um agendamentos Pendentes ou Atrasados e finalizar', async ({ page }) => {    
    ...
    const btnbtnSemana = page.locator('button, .q-btn, [role="button"]').filter({ hasText: /^Esta semana$/i }).first();
    await expect(btnbtnSemana).toBeVisible({ timeout: 30000 });
    await btnbtnSemana.click({ force: true });
    await page.waitForTimeout(2000);
    ...
    const btnGravar = page.getByText(/Finalizar|Gravar|Salvar|Cadastrar|Registrar cliente/i).first();
